...
class ShapeBorderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()
      ...
      ..strokeWidth =
          width *
          2 // Multiply by 2 because stroke is centered on the path
...
class FreestyleCustomScreen extends StatefulWidget {
  ...
  const FreestyleCustomScreen({super.key, this.initialSource});
...
class _FreestyleCustomScreenState extends State<FreestyleCustomScreen> {
  ...
  void initState() {
    ...
    if (widget.initialSource != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _addPhoto(widget.initialSource!);
      });
    }
  }
  ...
  Future<void> _addPhoto(ImageSource source) async {
    ...
    if (p != null) {
      setState(() {
        photos.add(
          SimplePhoto(id: DateTime.now().toString(), image: File(p.path)),
        );
      });
    }
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SingleChildScrollView(
                        ...
                        child: Row(
                          children: ['Roboto', 'Serif', 'Monospace'].map((
                            font,
                          ) {
                ...
                child: Stack(
                  children: [
                    ...
                    for (var photo in photos)
                      _buildMoveableItem(photo, _renderFramedImage(photo)),
                    for (var text in textItems)
                      _buildMoveableItem(text, _buildTextCore(text)),
                  ],
                ),
              ),
            ),
        ...
        child: GestureDetector(
          ...
          onTap: () {
            if (item is SimplePhoto) {
              setState(() {
                int nextIndex =
                    (item.shape.index + 1) % PhotoShape.values.length;
          ...
          child: Transform.rotate(
            angle: item.rotation,
            child: Transform.scale(scale: item.scale, child: content),
          ),
        ),
      ),
    );
  ...
  Widget _buildStaticDustbin() {
    bool isHovering =
        _currentFingerPos.dy > MediaQuery.of(context).size.height * 0.82;
    return Align(
            ...
            child: Column(
              children: [
                ...
                CollageControlPanel(
                  style: myStyle,
                  onColorChanged: (c) =>
                      setState(() => myStyle.borderColor = c),
                  onWidthChanged: (w) =>
                      setState(() => myStyle.borderWidth = w),
                  onBackgroundChanged: (b) =>
                      setState(() => myStyle.activeBackground = b),
                ),
              ],
            ),
          ),
